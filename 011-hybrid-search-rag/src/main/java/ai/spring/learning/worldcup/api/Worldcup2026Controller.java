...
/**
 * Same collapsed {@code /chat} route as every module since {@code 008-chat-memory}: {@link HybridSearchAdvisor} grounds it, served by a single model
 * {@code geminiChatClient}. {@code /chat/{conversationId}} is new: a plain JSON transcript read straight from Postgres, no LLM call, recording every
 * question and answer regardless of what {@code MessageChatMemoryAdvisor}'s windowed memory still holds.
 */
@RestController
