...
import ai.spring.learning.worldcup.advisor.HybridSearchAdvisor;
...
public class GenAiConfig {
	...
	private static ChatClient.Builder enrichChatClientBuilder(final ChatClient.Builder builder,
			final HybridSearchAdvisor hybridSearchAdvisor,
			final MessageChatMemoryAdvisor messageChatMemoryAdvisor) {

		final Advisor[] advisors = {new PiiRedactionAdvisor(10),
				messageChatMemoryAdvisor, // order 20
				hybridSearchAdvisor, // order 25
	 ...
	 */
	@Bean
	public HybridSearchAdvisor hybridSearchAdvisor(final VectorStore vectorStore,
			...
			@Value("${worldcup.hybrid-search.dense-top-k}") final int denseTopK,
			@Value("${worldcup.hybrid-search.sparse-top-k}") final int sparseTopK,
			@Value("${worldcup.hybrid-search.fused-top-k}") final int fusedTopK,
			@Value("${worldcup.hybrid-search.reciprocal-rank-fusion-k}") final int reciprocalRankFusionK) {
		return new HybridSearchAdvisor(vectorStore, jdbcTemplate, tableName, fullTextColumn, denseTopK, sparseTopK, fusedTopK, reciprocalRankFusionK, 25);
	}
	...
	public ChatClient geminiChatClient(final ChatClient.Builder builder,
			...
			final MessageChatMemoryAdvisor messageChatMemoryAdvisor,
			final HybridSearchAdvisor hybridSearchAdvisor) {
		return enrichChatClientBuilder(builder, hybridSearchAdvisor, messageChatMemoryAdvisor)
