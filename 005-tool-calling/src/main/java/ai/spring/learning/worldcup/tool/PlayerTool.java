 ...
 * the Match awards are only populated for players verified against {@link MatchTool}'s group-stage data (the goal/assist leaderboard and Man of the
 * Match winners, see {@link LeaderboardTool}); every other player keeps goals/assists/ cards null and Man of the Match awards at 0, since no verified
 * per-player stat exists for them. Squad data is split across several {@code addGroupX} methods (one per World Cup group) rather than a single map
 * literal, because a single static initializer for ~1,250 entries would exceed the JVM's 64KB bytecode-per-method limit.
 */
@Component
public class PlayerTool {
	...
	@Tool(name = "get-player",
			description = "Get a World Cup 2026 player's stats: goals, assists, cards and Man of the Match awards during the group stage")
	public Player getPlayer(@ToolParam(description = "The player's full name, e.g., 'Kylian Mbappé'", required = true) String name) {
