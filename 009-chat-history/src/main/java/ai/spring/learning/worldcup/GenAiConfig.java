...
public class GenAiConfig {
	...
	private static ChatClient.Builder enrichChatClientBuilder(final ChatClient.Builder builder, final PiiRedactionAdvisor piiRedactionAdvisor) {
		...
		return builder.defaultSystem("""
				...
				""").defaultAdvisors(AdvisorParams.ENABLE_NATIVE_STRUCTURED_OUTPUT).defaultAdvisors(advisors);
	}
	...
	public ChatClient geminiMemoryAwareChatClient(final ChatClient geminiMemoryNotAwareChatClient,
		...
		return geminiMemoryNotAwareChatClient.mutate().defaultAdvisors(messageChatMemoryAdvisor).build();
	}
	...
	public ChatClient geminiMemoryNotAwareChatClient(final ChatClient.Builder builder,
		...
		return enrichChatClientBuilder(builder, piiRedactionAdvisor).defaultOptions(getGeminiChatOptions(model)).build();
	}
}
