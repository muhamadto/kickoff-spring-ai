...
import ai.spring.learning.worldcup.query.StepBackQueryService;
import ai.spring.learning.worldcup.query.StepBackSearchService;
 ...
 * reaches the step-back-augmented RAG chat {@code 014-query-optimised-rag} introduced, and only {@link MessageClassification#BOOKING_REQUEST} reaches
 * the booking path.
 *
 * <p>
...
public class Worldcup2026Service {
	...
	private static String systemPrompt(final String stepBackQuery, final Optional<String> stepBackContext) {
		...
		return BASE_SYSTEM_PROMPT + """
				...
				%s
				---------------------
				""".formatted(stepBackQuery, stepBackContext.get());
	}
	...
	private BookingRequest extractBookingRequest(final String question) {
		return classificationChatClient.prompt()
				.system("""
