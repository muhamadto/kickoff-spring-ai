...
/**
 * {@code /chat} is unchanged from {@code 014-query-optimised-rag}. {@code /chat/{conversationId}} is new: a plain JSON transcript read straight from
 * Postgres, no LLM call. It sits next to {@code /chat} rather than being folded into it, since it answers a different question ("what was said in
 * this conversation", an audit question) from the one {@code /chat} answers ("what's the answer to this new question", a generation question). Named
 * for what it returns, not nested as a sub-resource of anything.
 */
@RestController
